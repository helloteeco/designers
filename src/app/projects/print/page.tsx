...
function PrintBriefContent() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-8 py-12 font-sans text-gray-900 print:px-0 print:py-0">
      ...
      {settings && (settings.studioName || settings.studioLogoUrl) && (
        <div className="mb-8 pb-4 border-b border-gray-200 flex items-start justify-between">
          <div className="flex items-center gap-3">
            {settings.studioLogoUrl && (
              /* eslint-disable-next-line @next/next/no-img-element */
              <img src={settings.studioLogoUrl} alt="" className="h-10 w-auto" />
            )}
            <div>
              <div className="font-bold text-gray-900">{settings.studioName}</div>
              <div className="text-[10px] text-gray-500 leading-tight">
                {settings.studioEmail}
                {settings.studioEmail && settings.studioPhone && " · "}
                {settings.studioPhone}
              </div>
            </div>
          </div>
          <div className="text-right text-[10px] text-gray-500">
            Design Brief · {new Date().toLocaleDateString()}
          </div>
        </div>
      )}
      ...
      {settings?.briefFooterNote && (
        <div className="mt-12 pt-6 border-t border-gray-200 text-sm text-gray-700 italic text-center max-w-2xl mx-auto">
          {settings.briefFooterNote}
        </div>
      )}

      {/* Footer */}
      <footer className="mt-6 pt-4 border-t border-gray-200 text-xs text-gray-400 text-center">
        {settings?.studioName ? (
          <>
            {settings.studioName}
            {settings.studioEmail && ` · ${settings.studioEmail}`}
            {settings.studioPhone && ` · ${settings.studioPhone}`}
            {settings.studioWebsite && ` · ${settings.studioWebsite.replace(/^https?:\/\//, "")}`}
          </>
        ) : (
